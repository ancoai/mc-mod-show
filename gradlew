...
if [ ! -f "$WRAPPER_JAR" ]; then
    if command -v gradle >/dev/null 2>&1; then
        warn "Gradle wrapper JAR is missing; falling back to system Gradle."
        exec gradle "$@"
    else
        die "ERROR: Missing Gradle wrapper JAR at $WRAPPER_JAR and no 'gradle' executable found on PATH."
    fi
fi
...
if $cygwin ; then
    [ -n "$JAVA_HOME" ] && JAVACMD=`cygpath --unix "$JAVACMD"`
    [ -n "$CLASSPATH" ] && CLASSPATH=`cygpath --path --unix "$CLASSPATH"`
    [ -n "$APP_HOME" ] && APP_HOME=`cygpath --unix "$APP_HOME"`
fi

if $msys ; then
    [ -n "$JAVA_HOME" ] && JAVACMD=`(cd "$JAVA_HOME" && pwd -W)/bin/java`
    [ -n "$CLASSPATH" ] && CLASSPATH=`cygpath --path --mixed "$CLASSPATH"`
    [ -n "$APP_HOME" ] && APP_HOME=`(cd "$APP_HOME" && pwd -W)`
fi
